...
**LLM:** по умолчанию используется Azure OpenAI (GPT-4o) + [OpenRouter](https://openrouter.ai/) как fallback. Через OpenRouter можно подключить любую модель — OpenAI, Anthropic Claude, Google Gemini и др. Настраивается через переменные окружения, код менять не нужно.
